...
Item {
    ...
    Process {
        id: monitorProc
        command: ["sh", "-c", "export PATH=$PATH:$HOME/.local/bin:/usr/local/bin:/usr/bin:/bin && hyprctl monitors -j"]
        running: false
    ...
    function applyWallpaper(safeFileName, isVideo) {
                ...
                const downloadScript = `
                    ...
                    if [ -n "$URL" ]; then
                        ...
                        if file "$DEST_FILE.tmp" | grep -iq "webp"; then
                            (magick "$DEST_FILE.tmp" "$DEST_FILE" 2>/dev/null || convert "$DEST_FILE.tmp" "$DEST_FILE" 2>/dev/null)
                            rm -f "$DEST_FILE.tmp"
                        else
                        ...
                        (magick "$DEST_FILE" -resize x420 -quality 70 "$FINAL_THUMB" 2>/dev/null || convert "$DEST_FILE" -resize x420 -quality 70 "$FINAL_THUMB" 2>/dev/null || true)
        ...
        const fullScript = `
            if ! pgrep -x swww-daemon >/dev/null; then swww-daemon & sleep 0.2; fi
