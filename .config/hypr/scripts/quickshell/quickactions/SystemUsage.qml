...
Item {
    ...
    property var requestedLayoutTemplate: [
        { x: 0.0, y: 0.0, w: 0.25, h: 0.5 },
        { x: 0.25, y: 0.0, w: 0.25, h: 0.5 },
    ...
    // GPU animated bindings
    property real rawGpuTemp: isNaN(SysData.gpuTemp) ? 0.0 : SysData.gpuTemp
    property real gpuTempC: rawGpuTemp
    Behavior on gpuTempC { NumberAnimation { duration: 800; easing.type: Easing.OutQuint } }

    property real rawGpuUsage: isNaN(SysData.gpuUsage) ? 0.0 : SysData.gpuUsage / 100.0
    property real gpuUsage: rawGpuUsage
    Behavior on gpuUsage { NumberAnimation { duration: 800; easing.type: Easing.OutQuint } }
    ...
    Item {
        ...
        // --- Top Row: CPU, RAM, CPU TEMP, GPU TEMP ---

        LiquidSquare {
            x: root.cellX(0.0)
            y: root.cellY(0.0)
            width: root.cellW(0.0, 0.25)
        ...
        LiquidSquare {
            x: root.cellX(0.75)
            y: root.cellY(0.0)
            width: root.cellW(0.75, 0.25)
            height: root.cellH(0.0, 0.5)
            
            value: Math.max(0.0, Math.min(1.0, root.gpuTempC / 100.0))
            colorFill: root.cGreen
            icon: "\uF2C9"
            title: "GPU °"
            valueText: Math.round(root.gpuTempC) + "°"
        }

        // --- Bottom Row: DISK, NET, GPU USAGE ---
        ...
        LiquidSquare {
            x: root.cellX(0.666)
            y: root.cellY(0.5)
            width: root.cellW(0.666, 0.334)
            height: root.cellH(0.5, 0.5)
            
            value: root.gpuUsage
            colorFill: Qt.lighter(root.cGreen, 1.2)
            icon: "\uF26C"
            title: "GPU"
