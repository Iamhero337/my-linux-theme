...
if [[ "$ACTION" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$TARGET" == "move" ]]; then
        hyprctl dispatch "hl.dsp.window.move({ workspace = $ACTION })" >/dev/null 2>&1 || hyprctl dispatch movetoworkspace "$ACTION" >/dev/null 2>&1
    else
        hyprctl dispatch "hl.dsp.focus({ workspace = $ACTION })" >/dev/null 2>&1 || hyprctl dispatch workspace "$ACTION" >/dev/null 2>&1
    fi
    exit 0
fi
    ...
    (
        ...
        while IFS= read -r filename; do
            ...
            if [[ "${extension,,}" == "webp" ]]; then
                new_img="${img%.*}.jpg"
                (magick "$img" "$new_img" 2>/dev/null || convert "$img" "$new_img" 2>/dev/null) && rm -f "$img"
                img="$new_img"
            ...
            if [[ "${extension,,}" =~ ^(mp4|mkv|mov|webm)$ ]]; then
                ...
                if [ ! -f "$thumb" ]; then
                    (magick "$img" -resize x420 -quality 70 "$thumb" 2>/dev/null || convert "$img" -resize x420 -quality 70 "$thumb" 2>/dev/null)
