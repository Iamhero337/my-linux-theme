...
if [ ${#MISSING_CMDS[@]} -ne 0 ]; then
    notify-send -u critical -a "Screenshot System" "Missing Dependencies" "Cannot start screenshot tool. Please install:\n${MISSING_CMDS[*]}"
    exit 1
fi
# ---------------------------------------------------------
...
if [ "$FULL_MODE" = true ] || [ -n "$GEOMETRY" ]; then
    ...
    if [ "$RECORD_MODE" = true ]; then
        ...
        # Execute screen recorder with wf-recorder fallback
        if command -v gpu-screen-recorder &> /dev/null; then
            ...
            [ -n "$GEOMETRY" ] && WF_ARGS+=(-g "$GEOMETRY")
            [ "$DESK_MUTE" != "true" ] && WF_ARGS+=(-a)
            wf-recorder "${WF_ARGS[@]}" > /dev/null 2>&1 &
            REC_PID=$!
        else
            notify-send -u critical -a "Screen Recorder" "Error" "Neither gpu-screen-recorder nor wf-recorder is installed."
            exit 1
        fi
